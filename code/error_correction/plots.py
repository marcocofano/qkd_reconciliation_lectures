...
from utilities import mpl
import utils


mpl.init(font_size=12, legend_font_size=12, tick_size=12)
...
def plot_(pairs, label, style=None):
    ...
    if style is None:
        plt.plot(*argsl, **kwargs)
    else:
        plt.plot(*argsl, style, **kwargs)
...
def plot_all(dl):
    for r in dl:
        plot_(r.data[args.error], r.get_label())
    fmt_err()
    plot_common()
...
def ensemble(dl):
    for r in dl:
        plot_(r.data[args.error], None, "r--")
...
def hist_iter(dl):
    ...
    mpl.fmt_ax(ax, "Number of iterations", "Frequency", leg=1, grid=1)
...
def avg_iter(dl):
    ...
    mpl.fmt_ax(plt.gca(), xlab, ylab, leg=1, grid=1)
...
def plot_common(title=None):
    ...
    mpl.save_show_fig(args, plt, img_path)


def fmt_err():
    xlab, ylab = x_labels[args.channel], args.error.upper()
    mpl.fmt_ax(plt.gca(), xlab, ylab, leg=1, grid=1, grid_kwargs={"which": "both"})
    plt.yscale("log")
...
def main(args):
    ...
    file_names = file.filter_strings(args, utils.get_data_file_list(args.data_dir))
    if not file_names:
        exit()
    labels = file.gen_unique_labels(file_names, tokens=["_", "__", "-", ".json"])
    data_list = [DataRoot(fn, lb) for fn, lb in zip(file_names, labels)]
    data_list.sort(key=lambda it: file.naturalkey(it.get_label()))
...
def setup_parser():
    ...
    parser.add_argument(
        "--type", help="type of the graph", choices=plot_reg.keys(), default="plot_all"
    )
    parser.add_argument("--param", help="parameter used in hist_iter graph", type=float)
    parser.add_argument(
        "--error", help="error rate metric", default="ber", choices=["wer", "ber"]
    )
    parser.add_argument(
        "--group_regex",
        nargs=2,
        action="append",
        help="if --type regex_average, matches a regex to file names and only plot their average. format: [<regex>, <legend name>]",
    )
    ...
    parser.add_argument(
        "--legend_format", help="format of legend entries", choices=legend_reg.keys()
    )
    parser.add_argument("--title", help="graph title", type=str)
    parser.add_argument("--file_name", help="save name", type=str, default="graph")
    parser.add_argument(
        "--agg", help="set matplotlib backend to Agg", action="store_true"
    )

    mpl.bind_fig_save_args(parser)
    file.bind_filter_args(parser)
